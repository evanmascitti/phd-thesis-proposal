...
However, I feel that as a group, scientists do not give enough attention to the _actual mechanics of how work is achieved_.
...
The purpose of this section is to transparently describe the practices I have adopted in my day-to-day work.
They are relevant to this proposal because these habits help ensure the results of my work are correct and reproducible.
I want my discoveries to be useful both to others and to my future self. This requires some best practices as the data are collected, stored, and analyzed. 
...
Hadley Wickham's "tidy" data framework [@Wickham2014] also factors heavily in my data collection and storage routines. 

#### File naming conventions 

I am very cautious about how I name files.
At first blush this sounds like a trivial topic, but I actually think it is actually one of the most important and overlooked tasks in research.
...
It's likely each output will use some of the same figures and table. That means that if we make any changes - for example, by adding new data or by changing some model parameters - the figure or table needs to be updated in multiple places. 
...
This script will read `clean-data.csv`, generate the plot, and save it as `fig-1.pdf`.^[I never use numbers when naming figures, because their order may change later. I use descriptive names instead. However, a number makes more sense to demonstrate the point in this minimal example.]

A user of GNU `Make` writes a plain-text file known as a `Makefile` containing a rule for each target. 
...
We might employ a manual version control system by as naming files successively, like `report-4.docx.` Hopefully we avoid a situation like the comic below:
...
Indeed, when I first heard of ideas like drawing diagrams with code or ditching Word and Powerpoint, I was initially skeptical.
...
I became increasingly incensed at the notion of wasting my valuable time collecting data that might eventually vaporize. 
I decided something had to change. 

At first I was only interested in R because I saw my friends making pretty graphs with **ggplot2**, and I wanted to do that too - but I quickly became hooked on the idea of reproducible research and all the associated tools.

I am not a naturally organized person, and I used to think that programming was not for me. 
I have perhaps over-compensated for these innate tendencies and gone a bit further than necessary -- but it's been more than worth the effort.
I swear by these routines because they enhance my ability to do high-quality science. 
...
During frustrating moments of debugging \LaTeX or fighting with `Make`, I sometimes wonder if it is worth all the extra effort.
