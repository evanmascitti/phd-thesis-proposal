...
In this literature review I focus on three areas relevant to baseball infield soils:

**Section \@ref(infield-performance)** [**Surface performance of baseball/softball infields**](#infield-performance)

**Section \@ref(lab-methods-review)** [**Laboratory tests of soil behavior and physical properties**](#lab-methods-review)
...
The few existing studies the infield skin soils have focused on construction and maintenance rather than mix design. 
...
\begin{equation}
w = \frac{V_{water}}{V_{soil~solids}+V_{voids}}
...
This is not the case for $\theta$. Once all pores are filled with water, bulk soil volume increases as with further water additions if the soil cannot drain freely.
...
```{r example-phase-diagram, out.width='50%', fig.cap='Example phase diagram of ain infield sand-clay mix showing solid, liquid, and gaseous phases.', fig.scap='Example phase diagram for an unsaturated sand-clay mix.'}

soiltestr::ggphase_diagram(sand_pct = 0.6, clay_pct = 0.4, G_sa = 2.7, G_c = 2.7, dry_density = 2, water_content = 0.1)
...
Particle size analysis (PSA) refers to a series of tests which measure the distribution of individual sizes for a given soil.
PSA is inexpensive and relatively fast.
For these reasons it has been used as a surrogate marker for some property in nearly every sub-discipline of soil science. 
...
PSA has formed the basis for most discussions around infield mix (for example, @Schroder2009; -@Schroder2012). 
Current recommendations for infield mixes revolve around total sand content and silt-to-clay ratio (SCR) [@ASTMF2107-08]. 
There are two main problems with using PSA as a specification for infield soils:

1. Obtaining accurate PSA data is challenging.
1. The linkage between PSA and the behavior of fine-grained soils is spurious. 

When I began this project, I conducted an an extensive review of PSA literature.
I chose not to include it in this proposal because PSA is not the focus of this dissertation. 
...
Instead, I have included this section to briefly describe possible pitfalls of using PSA to characterize infield soils. 
...
Even when protocols are clearly reported, we are confronted with a difficult dilemma: if carbonates, iron oxides, silica cements, and organic matter are all removed, what exactly is the physical meaning of the test result? These components surely contribute to the soil's properties in its "natural" state.


ASTM D-7928 [-@ASTMD79282017] is the protocol in use by most U.S. labs. This document provides a clear disclaimer about the variability of natural soils and the need for context-specific methods:
...
@Gee2002 is considered an authoritative text on PSA methods. These authors note the large potential variation in clay-size content (emphasis mine):
...
A system which relies so heavily on un-standardized methods cannot provide an adequate means of assessment
...
The behavior of fine-grained soil is markedly affected by its water content [@Holtz2010]. Soil plasticity tests measure the unique water contents corresponding to shared behavioral thresholds.  This section reviews the tests' history, methodology, and potential shortcomings.
...
Note that in Atterberg limit tests the term **water content** is always used on a gravimetric basis ($w$).
...
```{r barnes-apparatus, fig.cap='Barnes apparatus used for determining plastic limit and soil toughness (\\protect\\hyperlink{ref-Barnes2013}{Barnes, 2013})', fig.scap='Barnes apparatus for determining plastic limit and soil toughness.'}
...
Toughness is a measure of work and it bears units of energy per unit volume ($\mathrm{\frac{J}{m^3}}$) [@Mamlouk2006]. 
...
Most research on soil behavior is concerned with stress and strain at failure. Less attention has been given to energy dissipation at moderate strain.
...
This observation is well-understood by people who work intimately with clay in trades such as pottery-making or earthworks. Atterberg himself noted that the "plasticity number" did not provide information on the toughness of the soil (Section \@ref(atterberg-limits-origins)).  Similarly, @Casagrande1932 noted:
...
@Casagrande termed the resulting formula was termed "toughness index":
...
There are no universally agreed-upon definitions of "clay" and "silt," although a number of academic and professional societies maintain their own official language (i.e. @Guggenheim1995). These terms' working definitions are context-specific; the qualities important to the earthworks engineer differ from that of the potter, which differ from that of the geologist and so on. 
...
This equation may be useful for a cursory investigation, but the idea that it could completely replace compaction testing seems unlikely.
