...
The liquid limit test described by @Atterberg1911 included an unacceptable degree of operator dependence because it required the operator to manually agitate the soil. A mechanical test method was first presented by @Casagrande1932. The Casagrande method uses a brass cup fixed to a rotating cam, which agitates the soil paste by dropping the cup against a hard rubber base of specified height. The method has undergone little alteration up to the present. 

The Casagrande apparatus must be frequently calibrated to ensure accurate results. The most likely sources of error are a worn grooving tool and variability in in drop height caused by a loose locking screw. However, provided a standard test device is used, differences in operator technique comprise a larger source of error than the device itself [@Sherwood1970].

A recent survey by @Haigh2016 supports Casagrande’s warning that the stiffness of the rubber base can significantly affect the test result.
