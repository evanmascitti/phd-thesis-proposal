...
The new chart eliminated the A-line and instead defined two lines to separate clays from intermediate soils and intermediate soils from silts. The lines were drawn on the basis of toughness computed from their equation and re-linked to the LL and PL of a soil.  
...
Gravel, sand, soil, coal ash, and lime are commonly components. 

Soil mixtures for baseball and softball fields typically contain two materials - a sand and a fine-grained soil.
Table \@ref(tab:soil-mixture-study-list) comprises a selection of studies which tested mixes containing sand plus a single soil (2-component mix) or two types of soil (3-component mix). 
Table \@ref(tab:soil-mixture-study-list) also shows that researchers have evaluated the mixtures using a variety of analyses. These include Atterberg limits, compaction, shear strength, permeability, static compressibility, and pore size distribution.
...
This section describes published research pertaining to the behavior of sand-soil mixtures. 
...
At this point the soil’s behavior is dominated by the fines and the coarse grains act only as inert inclusions. 
The percent fines at which the soil's behavior changes from that resembling the 'host' sand to that of the fines has been termed the “transitional fines content.” The transitional fines content (TFC) is relevant to most infield mixes because they have fines content near the TFC.
...
However, others have advocated for enumerating the amount of sand to the amount of clay. For example, if 40 grams sand are added to 100 grams clay, this mixture would be termed 40% sand @[Briish clays textbook]. 
...
- There is no universal value for the TFC (for example, 70% sand by mass). 

- Factors affecting the TFC value in a binary mixture include sand gradation, fines plasticity, compaction energy, and behavioral parameter of interest.
...
## Behavior of artificial soil mixtures 

This section summarizes research about how the components of a soil mixture affect its behavior.
Researchers have used a variety of methods to evaluate soil mixtures. 
This review focuses on studies which have measured Atterberg limits, shear strength, or toughness because these properties are the most relevant concerns on baseball and softball infields.
Two-component mixtures (sand with a single type of clay) and three-component mixtures (sand with two forms of clay) are both examined. 
