...
**\@ref(artificial-soil-mixtures)** [**Design and behavior of artificial soil mixtures**](#artificial-soil-mixtures)
...
Soil composition, density, and water content act together to govern the stiffness of the surface. 
Surface deformation attenuates linear and rotational kinetic energy of the ball.
Existing studies on the infield skin have focused on construction/maintenance practices rather than mix design. 
...
This section reviews literature on the laboratory tests most pertinent to infield soils.
The first two sub-sections briefly cover soil water content and particle size analysis.
The latter two sub-sections provide a more extensive treatment of two test categories most relevant to infield soils: plasticity tests and toughness tests.
...
This brief section does not attempt to review the overwhelming body of literature on soil water content. 
It simply defines the main connotations in which the term "water content" is used. 
...
(2.) Volumetric ($\theta$): volume of water referenced to total volume of soil solids plus voids

A "true" value for $\theta$ is readily computed from $w$ if soil's dry bulk density $\rho_{bulk}$ is also known ($\theta=w~\cdot~\rho_{bulk}$).

$\theta$ is commonly estimated in-situ with indirect techniques such as neutron scattering or time-domain reflectometry [@Brady2007].

Both $w$ and $\theta$ are used across disciplines. 
Each has merits and downfalls. 
$w$ is easiest to measure: it simply requires an accurate balance and a drying oven. 
$\theta$ has a clearer physical meaning in terms of soil behavior and the interactions among particles. 
...
Soils consist of solid particles having a range of sizes. 
Particle size analysis (PSA) refers to a series of tests which measure the distribution of particle sizes for a given soil.
PSA is inexpensive and relatively fast to perform. 
As a result it has been used as a surrogate marker for some property in nearly every sub-discipline of soil science. 
Unfortunately the results of a PSA are highly dependent on the method employed, and there is no universally-accepted protocol.

PSA has been the focus of most discussions around infield mix [@Schroder2009; @Schroder2012]. 
Current recommendations for infield mixes revolve around total sand content and silt-to-clay ratio (SCR) [@ASTMF2107-08]. 
There are two main problems with using PSA as an indicator of infield performance:

1. Obtaining accurate PSA data is challenging.
1. The linkage between particle size and soil behavior is spurious. 

The author has conducted an an extensive literature review of PSA literature. I opted not to include it in this proposal because PSA is not the focus of this dissertation. 
I felt that a long discussion around PSA methods would divert the reader's focus away from more central themes of the project. 
Instead I have included this section to briefly summarize the current usage of PSA for infield mixes and voice some concerns I have regarding the use (and abuse) of PSA around this topic. 
Rather than recount the minutiae of particular studies, I focus on the sentiments of governing authorities and the views of historically significant soil scientists.  


#### PSA methods 

There is no universally agreed-upon method for performing a PSA - many samples requires special treatment, making comparison of results difficult. 
Authors rarely report the full protocol used for their PSA results. 
The vast array of possible combinations of pre-treatement, dispersion, and measurement techniques mean that comparing PSA results across studies is difficult or impossible. 

@ASTMD79282017 is the current accepted protocol for most U.S. labs. This document provides a clear disclaimer about the variability of natural soils and the need for context-specific methods:
...
@Gee2002 is considered an authoritative text on PSA methods. These authors note the large potential variation in clay-size content:

>Depending on the method of chemical treatment and physical dispersion used, measured clay content for an individual soil sample can vary by factors of four or more.

The above statements imply that the a single infield mix could easily pass a PSA specification when tested with one method but fail when tested with another - even when tested without error in the same lab, by the same operator.
A system which relies so heavily on un-standardized methods cannot provide an adequate means of classification.


#### Relationship between PSA and soil behavior 

The behavior of fine-grained soil is governed not just by its particles' size but also by their surface charge and shape.
There can be wide disparities in the behavior of two soils having very similar textural classifications.
...
- @Casagrande1947 compared agricultural classification systems based on grain size with his Airfield classification developed during World War II. He pointed out the main pitfall of a textural classification system:
...
- @Terzaghi1996 discussed the importance of choosing the correct soil tests during site characterization: 
...
- @Blackall1952, described the views of A. Atterberg, whose work formed the basis of both the grain-size system used in agricultural soil science _and_ the plasticity-based system used by engineers:

>“He decided...that very often the mechanical composition obtained from settling analysis did not express the physical properties of the soil. So he set out to determine the physical properties peculiar to the different soil types, and found plasticity to be particularly characteristic of clay.”



#### Summary: particle size analysis 

PSA is a useful starting point in predicting a soil's physical behavior and how it will interact with water. 
This forms the basis for the USDA textural triangle.
However PSA is difficult to measure accurately. 
In addition, PSA cannot always predict how a soil will respond to changes in water content. 
...
Developing a framework more grounded in actual soil behavior is the primary motivator of this dissertation.
...
He considered the effort needed to remold the soil a separate property, which translates to English as "viscosity." This distinction will be considered further in Section \@ref(soil-toughness).
...
Note that in Atterberg limit tests the term **water content** is always used on a gravimetric basis.
...
Reproducibility is the similarity of results for the same sample by multiple test operators or multiple labs [@Ullman2009]. 
...
The goal was to compare the variation across each means of replication. 
...
One could expect better agreement for the liquid limit and this is clearly borne out in Figure \@ref(fig:sherwood-cov-comparison).
...
@Sherwood1970 showed that most soil physical tests are repeatable only when performed by qualified operators with a well-tuned apparatus and careful workmanship.

It is next useful to explore the sources of these errors so they can be avoided as much as possible. Four chief sources of laboratory error for the Atterberg limits include:
...
Mixing water with the sand prior to adding the bentonite increased $w_{opt}$ by up to 2% compared with mixing the soil components dry and adding the water last. $\rho_{max}$ was also lower when the water was added before the bentonite. 
...
They reported a 4 % water content decrease of in the liquid limit due to oven-drying at either 60 or 110&deg; C. 
This was attributed to difficulty in re-hydrating the clay mineral surfaces and micropores. 
If the soil was oven-dried, the liquid limit could be increased by curing the sample for 24 hours after adding water. 
...
Evidently the cements were degraded by the extra agitation because non-cemented London clay showed no change after additional mixing. 
...
The loose specification led to disparities among the devices produced by various manufacturers, and these yielded different liquid limits despite being otherwise identical. 
...
###### Technique errors: plastic limit
...
To clarify, these data points are not the liquid limit - they are specimens removed from a bowl of soil having (ostensibly) homogenous water content. 
Each point represents the water content determined from a ~5 g sample from the same bowl of soil.
The samples were taken in rapid succession and could be considered identical "technical replications."
Figure \@ref(fig:LL-w-determination-variability)  indicates that even with the use of a milligram balance, one should expect a water content standard deviation of ~ `r LL_sd` when sampled from a well-mixed specimen.
...
The device consists of a flat plexiglass plate which is used to roll the thread instad of the operator’s hand. @Bobrowski1992 also stated that paper should be affixed to the base of the device to prevent the thread from sliding and to expedite the drying process. 
...
The thread is carefully bent about its center, which is anchored around a stainless steel cylinder. When the thread begins to crack, a caliper is used to measure the distance between the two ends of the thread (\@ref(fig:moreno-marato-thread-bending-procedure)). 
...
```{r moreno-marato-thread-bending-procedure, fig.cap='Thread-bending test described by Moreno-Maroto and Alonso-Azcárate (2015)'}
...
The authors argued that their test meets the original requirements of any plastic limit test which could replace the current method, namely:
...
Many attempts have been made to improve or supplant the canonical test methods.
...
Many fine-grained soils exhibit ductile behavior, but their strength in this state varies widely. 
This observation is well-known by those who work intimately with clay in trades such as pottery production or earthworks. Atterberg himself noted that his "plasticity number" did not provide information on the toughness of the soil (Section \@ref(atterberg-limits-origins)).  Similarly, @Casagrande1932 noted:
...
1. New characteristic water contents and indices can be defined in terms of toughness. For example, the toughness coefficient is the rate of change in toughness as a function of water content ($\frac{\text{d}T}{\text{d}w}$).
...
3. The addition of coarse particles decreased toughness by diluting the clay content. For equal 'matrix water content' (i.e. water per unit clay-size material) coarse particles _increased_ toughness because of the increased work required to shear them through the viscous clay matrix. However, maximum toughness was lower for mixed soils because the presence of coarse particles brought the onset of brittle failure at a higher water content due to interparticle interference. The total effect of coarse additions is to raise the plastic limit relative to the pure clay and reduce maximum toughness.
4. Some clays undergo strain-hardening during the rolling procedure, while others exhibit strain-softening behavior. 
5. The toughness of clay mixtures does not always obey a linear law. In other words, the behavior of an artificial mixture containing two clays will not necessarily reflect the mixture's proportions. 
...
A baseball field differs because a small amount of deformation to the soil is desirable (Section \@ref(corkboard-effect-section)). 


#### Computing toughness from index properties

@Casagrande1932 commented further on the relationship of toughness to soil testing and classification:
...
@Moreno-Maroto2018 correlated the plastic and liquid limits with maximum toughness ($T_{max}$) measured by @Barnes2013. By re-arranging their equation, toughness is computed as:
...
The new chart eliminated the A-line and instead defined the C-line and M-line to separate clays from intermediate soils and intermediate soils from silts. The lines were drawn on the basis of toughness computed from their equation and converted to LL and PL values.  
They also pointed out that the slopes of their C-line and M-line closely match the toughness groups given by @Barnes2009.
...
_Any_ attempt to classify soils into groups invokes a degree of arbitrariness. A pedantic discussion of "silt-like" or "clay-like" behavior typifies the murky business of classifying natural entities into discrete bins. 
There are no universally agreed-upon definitions of "clay" and "silt." These terms' working definitions are context-sepcific; the qualities important to the earthworks engineer differ from that of the potter, which differ from that of the geologist and so on. 
...
Such an equation may be useful for a cursory investigation, but the idea that it could replace compaction testing seems spurious. 

#### Summary: toughness research

The toughness of soil has not been studied extensively. 
...
The goal of laboratory soil testing is to provide information about a soil's 
...
Particle size analysis is the only soil testing method which has been applied to infield soils research. 
...
Little research on soil toughness is available but the few available studies suggest toughness is a useful concept for studying infield soils. 
...
In this section, the first topic outlined is the rationale for using engineered soils.
...
The soil's behavior becomes dominated by the fines with the coarse grains acting only as inert inclusions. 
This threshold has been termed the transitional fines content (TFC). 
The global void ratio also increases with fines content > TFC because solid sand particles are displaced by a corresponding volume of fines and inter-fine voids.

Infield soils must exhibit plasticity for good performance during play but they cannot have such high strength that they cannot be worked with tools and implements.
In other words, an infield mix is _transitional_ between coarse and fine grained soils.
Therefore, the TFC is a relevant concept to the design of infield mixes.
The following sub-sections review literature on the TFC concept. 

#### Means to define the TFC 

It is important to acknowledge that the TFC is not a universal number. Most studies report values of 20-30% fines by dry mass, with extreme values ranging from about 15 to 50% [@Zuo2015; @Kenney1991; @Vallejo2000]. 
The TFC value varies across mixtures because (1) it depends on the behavioral threshold of interest (for example, compressibility, shear strength, or permeability), and (2) a unique ratio exists for every combination of coarse-grained and fine-grained components. 
The total state of particle packing depends on the grain-size distribution of each component, the plasticity of the fines, the means of compaction or consolidation, and the state of effective stress [@Zuo2015].

Researchers have defined the TFC using several methods (Table \@ref(tab:tfc-definition-methods)). A few examples include: calculations from phase relations and index properties [@Thevanayagam1998; @Monkul2007; @Cabalar2011; @Lade1998], the minimum global void ratio following compaction [@Lade1998; @Ueda2011], undrained shear strength during triaxial tests [@Thevanayagam1998; @Vallejo2000; @Cabalar2017; @Kim2018], parameters of the normal compression line or critical state line,(@Cabalar2011; @Carrera2011; @Shipton2012] and liquefaction potential [@Lade1997].

```{r tfc-definition-methods}
tfc_definition_methods <- readr::read_csv(
  "data/lit-review-data/tables/selected_transitional_fines_content_studies_one_per_row.csv",
  col_types = 'ccccc', na = '') %>% 
...
tfc_def_kable <- tfc_definition_methods %>% 
  ...
  column_spec(2, width = '1.1in') %>% 
  # column_spec(3, width = '2in', latex_valign = 'm') %>% 
...
The maximum possible density of a coarse-fine mixture occurs when the fines just fill the large voids in the sand skeleton.
This concept permits a simple estimate of the TFC.
The individual components are assumed to exist at their respective maximum densities (minimum void volumes). 
A convenient analytical solution for the global solids volume $V_s$ is:
...
where $n_{global}$ is the global porosity and $n_{coarse}$ and $n_{fines}$ represent the porosity of each component when packed to their respective maximum densities [@Lade1998]. 
For example, a sand having a minimum porosity of 0.35 and a clay having a minimum porosity of 0.4, the theoretical maximum solids volume or "packing fraction" of the mixture is $1-\left[0.35\cdot0.40\right]=0.86$ or 14% porosity.

Packing fractions measured in experiments are always lower than their theoretical maxima [@Zuo2015]. 
They range from 60% to 95% of the theoretical maximum [@Furnas1931; @McGeary1961; @Zuo2015]. 
The disparity can be attributed to the fact that simplifying assumptions of maximum density of each component and "infinitely small" fines are unlikely to be satisfied [@Zuo2015].  

The simple approach of defining the TFC as the % fines producing the minimum amount of vpids may not reflect the actual behavior of the soil. 
@Cabalar2011 showed that the behavior transition from sand-like to clay-like occurs at a fines content lower than predicted by the global void ratio. 
The intergranular void ratio (discussed below) may yield a more accurate prediction of the behavioral threshold.  


#####  Intergranular void ratio

A second mathematical means to define the TFC is the intergranular void ratio $e_{SA}$ [@Thevanayagam1998]. 
This parameter enumerates the configuration of large grains within a coarse-fine mixture. 
$e_{SA}$ is derived from phase relations (Fig __) by considering the solid fines as voids.
 
For any fines percent and water content, $e_{SA_{mixture}}$ is compared to the minimum void ratio of the pure sand $e_{min_{pure~sand}}$. 
When $e_{SA_{mixture}}$ exceeds $e_{min_{pure~sand}}$, soil behavior will no longer be governed by contacts between coarse grains because of their loose configuration.
Therefore the TFC is defined as the fines content where $e_{SA_{mixture}}=e_{min_{pure~sand}}$
...
The analytical solutions described above assume that the fines are infinitely small, fitting completely within the larger grains.

The size ratio between the coarse and fine components partially explains the discrepancy between calculated and experimental values [@Lade1998; @Ueda2011]. 
In Equation \@ref(eq:global-tfc-void-ratio), it is assumed that the fines are infinitely small. 
The size ratio $ R $ was given by @Lade1998 as the ratio between the mean grain size of each component:
...
@Lade1998 concluded that when $R$ < 7, any increase in the size ratio imparts a relatively steep increase in packing density (see Figure \@ref(fig:lade-1998-figure-7)).
...
Therefore, by increasing the plasticity of the fines, one would expect the maximum achievable density of a mix to occur at a lower fines content. Compared to a low-plasticity soil, more of the intergranular space is occupied by water in a zero-air voids condition ($S_e$ = 1). 
...
A soil compacted at its optimum water content typically retains air in 5-15% of its void volume ($S_e=0.85~\text{to}~0.95$) [@Holtz2010]. 
...
- There is no universal value for the TFC (for example, 70%);. Values range from 15-45 % by mass and are specific to a given mixture, specimen prepration method, and problem of interest. 
...
- Soil mixtures are described by mass, but descriptions of behavioral thresholds are best specified using volumetric relations. This accounts for the presence of void volumes and specific gravity differences between the solids.
...
The Atterberg limits are geared toward fine-grained soils but the presence of particles in the silt-size and sand-size regions also affect the test results. 
This sub-section covers research on the effect of coarse additions to finer-grained soils. 
...
This is a relatively narrow range which would preclude efficient particle packing (see Section \@ref(#tfc-size-ratio)). It is possible the uniformity would play a bigger role if coarser sand were used in the mixture.
...
@Barnes2018 studied the toughness of clay mixtures. Various ball clay were combined to produce two-component mixtures containing 0, 20, 40, 60, 80, and 100% of each component. A single ball clay was also mixed with a fireclay of low plastcity. All the clays were kaolinite-dominated, but detailed mineralogy data were not collected.

In 2 of the 3 mixture pairings, the plastic limits of the mixtures were suppressed compared to values predicted by the linear law of mixtures. The pure AT and KT ball clays had nearly identical plastic limits (~30%), but the plastic limits of mixtures containing these clays was always less than 30%. By lowering the plastic limit more than the liquid limit, the overall range of water contents where the mix was plastic was extended. Critically, the maximum toughness of the mixture (i.e. at its plastic limit) was also increased over that of either pure clay.



Ceramic textbooks :

Clay bodies for ceramic applications are commonly manufactured by adding a 1-3% bentonite {ref}. It is evident that a minute amount of one clay can drastically alter the behavior of a mixture. [ref???]

For equal sand contents, the plastic and liquid limits of the mixtures did not lie on a linear interpolation between those of the pure clay end-members.
...
The transitional fines content describes the change from soil behavior dominated by grain-to-grain contacts to behavior dominated by water content and plasticity or toughness. 
This threshold has been variously definied according to the problem of interest. There is no universal number to describe its position, which ranges from ~15-35% by mass. 
Dominant factors governing the TFC position for a pair of soils include the particle size ratio between them, the void volume of the host sand, and the the plasticity of the fines. 


If sand or silt is added to a clay, the amount, size, and shape of the coarse particles can all influence the mixture's behavior. 
Coarse, rotund particles have a small influence on a clay while finer and more irregular particles tend to raise both the plastic and liquid limits of the mix. 
Mixing one clay with another results in a mixture whose properties cannot be easily predicted. 

These findings collectively demonstrate the complexities which result from the artificial mixing of two or more soils.
This complexity demonstrates the importance of further research on the design of soil mixtures. 
...
1. The importance of particle size analysis for infield mixes has been championed without any systematic evaluation of soils having different PSDs or similar PSDs but different mineralogy.
1. There is no existing research on the cleat-in, cleat-out effect.
1. Existing research on sand-clay mixes has almost exlusively used sand with particle sieve diameters of < 425 μm. This arbitrary cutoff limits the applicability of existing data to infield mixes which often contain significant amounts of coarser sand. 
1. Most studies about the transitional fines content of sand have used non-plastic fines. Plasticity is an important part of a well-functioning infield soil so these studies are not directly applicable. 
1. The effects of clay mineralogy, coarse additions, and water content on soil toughness have only been explored using a single device which has limited scope.
