...
Today, engineered soils are used for agricultural, horticultural, environmental, engineering, and recreational purposes [@Leake2014].

Deliberate soil mixing also ensures a nearly identical soil can be repeatedly produced over time. 
If natural variation in raw material properties is encountered, the mixture can be adjusted so that its final properties do not vary.
...
The raw components incorporated into a mixture depend on its intended use. Materials which retain plant-available water and nutrients are favored for agricultural or horticultural operations. These include fired or expanded ceramic granules and various types of organic matter. Soil mixtures produced for engineering applications are usually limited to inorganic components Gravel, sand, soil, coal ash, and lime are the commonly used raw materials. 

Soil mixtures for baseball and softball fields typically contain two materials - a sand and a fine-grained soil. Table \@ref(tab:soil-mixture-study-list) comprises a selection of studies which tested mixes containing sand plus a single soil (2-component mix) or two types of soil (3-component mix). Table \@ref(tab:soil-mixture-study-list) shows that a variety of methods have been employed to evaluate the mixtures, including Atterberg limits, compaction, shear strength, permeability, static compressibility, and pore size distribution.
...
- From the above it is clear that there is no unique value for the TFC (for example, 70% sand by mass).

- descriptions of behavioral thresholds are better specified using volumetric relations of the solid constituents rather than mass-based values because of differences in speecific gravity of the soil particles and the relative density of the bulk mixture
 
- analytical solutions complimented with measured index properties (i.e. maximum and minimum void ratio of the host sand) provide a useful starting point or estimation, but experiments are required for a more precise solution.

There is no unique value for the TFC. The TFC is different for every combination of an an arbitrary sand and an arbitrary clay 
