...
Great effort is made by the grounds staff to sustain the infield in a playable condition. A host MLB franchise may lose >$500,000 if a game is canceled because refunds must be offered to fans, yet overhead stadium operating costs are fixed. Therefore, every attempt is made to keep the field in a playable condition and complete the game. At the youth level, most revenue is driven by concessions and tournament registration fees. Repeated postponements can undermine a facility's operating model. It is clear that playing games during and after rain events is critical for both professional and recreational operations.

```{r mlb-salaries, fig.cap='MLB athletes are valuable assets. Their safety must not be compromised by the playing surface.', fig.show='hold'}
source('supplemental_R_scripts/mlb_player_pay.R')
cowplot::plot_grid(salary_distribution_2019_plot, mlb_revenues_plot, align = "h")

```

Athlete safety is paramount to player well-being and also to the business success of a professional franchise. Figure \@ref(fig:mlb-salaries) shows the distribution of MLB player salaries for 2019 and the steady rise in their pay with time. The median MLB player salary in 2019 was \$`r median_2019_salary_millions` million, with `r 100*round((total_2019_million_dollar_players / n_players_2019), 2)`% of players earning over $1 million and `r total_2019_10_million_dollar_players` players earning over \$10 million. Athletes are the most valuable assets of an MLB franchise. A team cannot afford for them be injured by a poor playing surface. 
