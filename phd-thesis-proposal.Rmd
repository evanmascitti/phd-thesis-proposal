...
```{r player-locations, eval=T, fig.cap="Locations of players during a professional baseball game, note the paucity of players on turf (blue arrows) compared to bare soil/skin areas (red arrows).", fig.width=6.5, out.width="100%" }

knitr::include_graphics('images/illustrations/player_locations/player_locations.png')

```

```{r produce batted ball density plot}
...
```{r labor-per-area, eval=T, dev= "pdf", fig.cap= "The infield skin consumes a majority of inputs despite comprising less than 10 percent of the playing surface area."}
labor_per_area_plot
```


```{r eval=T, battedballs-maintenance, dev= "pdf", fig.cap= "A) Intensity of play on the infield skin. B) The infield skin consumes a majority of inputs despite comprising only ~8% of the playing surface."}
...
### Hypotheses
1. The "cleat-in, cleat-out" phenomenon (described in Section \@ref(#coarkboard-effect)) can be duplicated using a machine and a laboratory-sized soil specimen. 
2. The means by which the soil specimen is prepared will affect the test result. Therefore, a protcol must be developed which adequately emulates the real procedures used for construction and maintenance of infield skins. Furthermore, the method must simulate outdoor conditions to adjust the soil water content.  
3. Subjectivity can be minimized by relying on 3D scanning technology to measure the response of the soil. 
4. Morphpometrics may be classified by means of the property they depend on; each metric will be minimized at simlar water content but their sensitivity (i.e. _relative_ magnitudes) will differ in this order: surface-area > volume > curvature.  
...
```{r eval=FALSE, cleat-mark-device, out.width="50%"}
knitr::include_graphics(c('images/cleat-mark-device.png', 'images/nine-spike-pattern.png') )
