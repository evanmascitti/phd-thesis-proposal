...
knitr::include_graphics('images/cleat-mark-device-on-lab-bench.JPG')
...
cleatmark_qc_data <- readr::read_csv('../../cleatmarkmethod/ecmdata/raw-data/patton-mocha-cleatmark-test-specimen-prep_2021-04-08/cleatmark-cyl-prep-QC_2021-04-08.csv', col_types = 'Dcccddcdddc') %>%
	...
	left_join(soilmesh::chunk_cyl_dims, by = c('mold_ID' = 'cylinder_ID')) %>%
	soiltestr::add_w() %>%
	mutate(ambient_temp_c = 22,
				 Gs = Gs,
				 #mold_ID = cylinder_ID
				 ) %>%
	# rename(empty_mold_mass_g = cyl_w_plug_mass,
	# 			 cylinder_vol_cm3 = cyl_w_plug_volume,
	# 			 filled_mold_g = filled_cylinder_mass_g,
	# 			 ) %>%
  soiltestr::add_physical_properties(mold_dimensions = soilmesh::chunk_cyl_dims, cleat_mark = TRUE) %>% 
  dplyr::mutate(cylinder_ID = mold_ID) %>% 
	# soiltestr::add_physical_properties(mold_dimensions = soilmesh::chunk_cyl_dims) %>%
...
```{r lamp-height-calibration, fig.cap='Surface temperature for the 30 cm lamp height closely matches conditions observed on a sunny day in Pennsylvania (outdoor photo credit: Andrew Burnette)', fig.scap='Temperature beneath drydown apparatus c.f. outdoor conditions'}

magick::image_read('../../cleatmarkmethod/figs/manually-built/drydown-surface-temp-comparison.png') %>% 
  magick::image_trim() %>% 
  magick::image_write('../../cleatmarkmethod/figs/manually-built/drydown-surface-temp-comparison.png')
...
```{r sand-backfill-vs-3D-scanner-for-volumetric-w-determination-fig, fig.cap='(ref:sand-backfill-vs-3D-scanner-for-volumetric-w-determination-fig-caption)', fig.scap='$\\rho$ obtained from 3D scanning and sand replacement'}
...
```{r example-nrfi-vs-water-content-curve, fig.cap='(ref:example-nrfi-vs-water-content-curve)', fig.scap='Conceptual stages of infield soil behavior with reducing $\\theta$'}
...
```{r proctor-extruder, fig.cap='Bottle jack specimen extruder for preparing compression test specimens.', fig.scap='Bottle jack specimen extruder',out.width='50%'}
