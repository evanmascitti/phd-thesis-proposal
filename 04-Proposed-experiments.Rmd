...
knitr::opts_chunk$set(echo = F, warning = F, message = F, out.width = '90%', eval=F)
...
```{r cleat-mark-device, fig.cap="Left: The pneumatic apparatus used to simulate foot-to-surface interaction. Right: A typical 9-spike baseball cleat pattern.", fig.show="hold", fig.height=3, out.width="50%", eval=TRUE}
...
$$\left(\frac{0.60~-~0.08}{0.96~-~0.08}\right)~\times~(1+0.002)~\times43~kg~=~25.46~\text{kg}$$
...
(ref:proctor-vs-observed-infield-density-caption) For the same soil, dry density values observed on a professionally managed infield surface lie between those achieved with the Standard and Modified Proctor tests. 


```{r proctor-vs-observed-infield-density, fig.cap='(ref:proctor-vs-observed-infield-density-caption)', eval=TRUE}
knitr::include_graphics('./figs/pdf/proctor-vs-observed-infield-density.pdf')
```
...
(1) the minimum diameter needed to accommodate several baseball cleats, and (2) a commonly used specification for infield soil depth (10.2 cm or 4 in)
...
The resin has a bulk density of just ~0.6 Mg m^-3^ so the plugs reduce the total specimen mass to ~ 4.5 kg and maintainin a sufficient profile depth of 7.5 cm. 
...
Therefore the lift thickness was held constant and the number of lifts was simply reduced to 2. 56 blows per layer are used for the 15 cm-diameter mold. 
...
A single water content was derived for each soil x effort combination using the Normalized RFI as the surface metric of interest. 
These values are summarized in \@ref(tab:derived-water-contents-from-cleat-mark-test-effort-comparison-tab).
These tests were not replicated so they cannot be statistically compared, but it appears that the compaction effort does not markedly influence the test result.
...
kableExtra::kbl(derived_test_values_from_compaction_pilot_table, 
                ...
                caption.short = "Critical water contents from cleat mark test performed with 2 compaction efforts") %>% 
  kableExtra::add_header_above(header = c(" " = 1, "$\\\\theta_{critical}$" = 2), escape = FALSE) %>% 
...
1. Despite the lack of a statistical comparison, the "critical" water contents derived from the two methods were quite similar. The differences due to soil type appears to be larger than any difference due to compaction effort. 

2. Soil bulk density on an actual infield was higher than that from a Standard Proctor test. The prevailing opinion among grounds managers is that the infield should be as compacted as possible. Therefore, a realistic lab test should favor greater compaction wherever possible. 
...
A heavy hand-watering is usually the first action taken following grading and compaction. Evacuating as much air from the profile aids in settling the soil to a stable condition.
...
Finally, the variation in water content due to time is much larger than any heterogeneities across an individual specimen or across replications. 
For this particular soil, gravimetric water loss was essentially linear for the first 3 hours of drying. 
After > &thickapprox; 3 hours, the rate of loss decreased. 
This is a well-documented phenomenon in soil physics (citation needed). 
...
This experiment was important to determine the uncertainty around the water content measurement. 
Figure \@ref(fig:w-variation-across-space-and-time) shows the results of this calibration repeated for three replicate specimens. 
...
```{r heat-lamps-photo, fig.cap='Drydown manifold used to reduce water content prior to cleat-mark test', eval=TRUE}
knitr::include_graphics('./images/heat-lamps.JPG')
```


```{r water-content-derivatives-per-lamp-height, fig.cap='The first derivative of $w$ with respect to time indicates that the rate of water loss changed rapidly for the lowest lamp height. The 30 and 45 cm heights lost water at a steadier rate.', eval=TRUE}
knitr::include_graphics('../../cleatmarkmethod/figs/pdf/water-loss-derivatives.pdf')
```
 

```{r w-variation-across-space-and-time, fig.cap='$w$ was consistent across the surface of a single specimen and across replicate cylinders.', eval=TRUE}
...
```{r lamp-height-calibration, fig.cap='Surface temperature for the 30 cm lamp height closely matches conditions observed on a sunny day in Pennsylvania (outdoor photo credit: Andrew Burnette)', eval=FALSE}

knitr::include_graphics('../../cleatmarkmethod/figs/manually-built/drydown-surface-temp-comparison.png')

```
 
###### Water content study 3: precise measurement of $\theta$
...
```{r density-plug-imgs, fig.cap='Two methods for measuring the volume of a 15 mm-deep by 21 mm diameter soil plug: A. Sand replacement method with a miniature plastic cone. B. 3D scanning method with plug situated on a rest to provide clearance from turn table.', eval=TRUE}
...
Together these components were used to fill the cavity left by the soil plug (Figure \@ref(fig:).
The plug volume was calculated by converting the mass of sand used to volume and subtracting the known cone volume:

\begin{align}
\quad V_{plug} &= V_{sand_{~total}} - V_{cone} ; \quad V_{sand_{~total}} = \frac{m_{sand_{~total}}}{\rho_{sand}} \\
V_{plug} &= \frac{m_{sand_{~total}}}{\rho_{sand}} - V_{cone} \\
(\#eq:mini-density-sand-backfill-equation)
\end{align}

In method (2) a 3D scan of the core was recorded. 
3D scanning has been used to measure bulk density for irregularly-shaped soil clods [@Rossi2008]. The scanning process was similar to that described in Section \@ref(cleat-mark-specimen-3D-scanning).
...
Ultimately the plug was stationed atop a small hex nut, providing enough clearance so the soil could be differentiated from the turntable without allowing its position to shift during the turntable's rotation. 

The volume of the `.ply` mesh was determined using the `vcgVolume()` function in the **`Rvcg`** package. 
...
```
The distribution of scatter in Figure \@ref(fig:sand-backfill-vs-3D-scanner-for-volumetric-w-determination-fig) clearly shows the better precision of the scanner method. 
...
Recall that the ultimate utility of the density measurement is computing a volumetric water content from $w$. As an example, if the true gravimetric water content is 15%, the consequence of such a large error in the density measurement propagates to a $\theta$ standard deviation of `r round(sand_replacement_rho_sd*0.15, 3)`. This would appear an unacceptably large range if the cleat-mark analysis is to pinpoint $\theta_{crit}$ within < 1 % accuracy. 
...
This sequence is repeated twice more, rotating the specimen by 60&degree; after each test.
...
water content blah blah blah 

Penetrometer resistance is measured with a handheld device having a maximum capacity of 5 kg cm^-1^. 
Four ratings are taken and averaged to producea a representative value. 
Visual observations on the nature of deformation are noted. 
Finally, a color photograph of the sample is taken after the test.
...
The raw meshes generated by the device are oriented in a coordinate system relative to the scanner's detectors. Due to the specimen geometry the scans also contained incomplete fragments of the aluminum cylinders housing the soil. To allow meaningful comparison of the soil surfaces, the cylinder portion must be removed from the mesh and the surface must be oriented in real-world coordinates. 
These tasks may be done interactively with point-and-click software such as MeshLab [reference to VCG], but an interactive approach is neither scaleable nor reproducible. Instead, processing routine was written using the R Language for Statistical Computing [@R-core2021].  
This code was primarily based on the **`Rvcg`** and **`Morpho`** packages [@Schlager2017], which are in turn built atop the open-source C++ libraries maintained by the ISTI-CNR Visual Computing Lab (Pisa, Italy, <https://www.isti.cnr.it/en/>). 
Source code associated with the orientation and cleaning of the meshes are available in this paper's supplemental materials. 
Briefly, a 4x4 roto-translation matrix was recorded using a reference specimen with an undisturbed surface. This matrix was saved as an .rda data object and applied iteratively to each mesh from that particuar scanner orientation. The sample was then adjusted in the x-y plane based on its maximum x and y coordinate values. Any points greater than 70 mm from the mesh's center were removed by computing Bboolean values for each vertex solved from the basic equation for a circle with origin $(0,0)$  :
...
Therefore for a given vertex, if $x^2+y^2>70$ the vertex was removed from the matrix of coordinates. The result is shown in \@ref(fig:orienting-and-removing-cyl-fig)


(ref:orienting-and-removing-cyl-caption) A. Orientation of the raw mesh generated by 3D scanner. B. Orientation of the specimen after application of roto-translation matrix. C. Final orientation of mesh after removing extraneous portions of the mesh and performing secondary position adjustments. Junction of the three axes represents $(0,0,0)$.
...
The critical water content $w_{crit}$ was identified as the first local minimum as the soil moves from wet to dry (right to left on Figure \@ref(fig:example-nfri-vs-water-content-curve)).

(ref:example-nfri-vs-water-content-curve) An example of the characteristic water contents used to derive $w_{crit}$. Water contents above A generally yield little additional surface area. Water content B represents the state in which the soil is most 'vulnerable' to fragmentation. As water content reduces further toward point C, the soil hardens and the cleats are less able to penetrate the surface. 

```{r example-nfri-vs-water-content-curve, fig.cap='(ref:example-nfri-vs-water-content-curve-caption)'}
...
The choice of this point as $w_{crit}$ warrants a brief discussion behind the physical meaning of the curve's shape.
...
When the soil reaches $w_{crit}$ it begins to fragment into discrete pieces as the cleats penetrate the surface. 
...
A possible complication not depicted in Figure \@ref(fig:example-nfri-vs-water-content-curve) is that with further decreases in water content the soil becomes so hard that NFRI falls below its value at $w_{crit}.
...
For this reason, any points drier than the local maximum marked as point (B) in Figure \@ref(fig:example-nfri-vs-water-content-curve) are discarded prior to optimizing the curve to derive $w_{crit}$.


# Summary: cleat-mark method 

- The method described above provides an bjective means to choose a single water content for any infield soil. This value is termed $w_{crit}$.

- $w_{crit}$ is considered the lower boundary of the water contents suitable for play.
...
```{r sand-content-clustering, fig.cap='The sand contents for Experiment 1 in Chapter 2 will be deliberately clustered around the transitional fines content to provide the best data resolution', eval=TRUE}
...
The soils will be wetted to their adhesion limit and packed into a 5-cm diameter steel tube with a dynamic compaction effort of 2,700 kN-m m^3^ (equivalent to the ASTM modified Proctor compaction test, D1557-12e1 [-@ASTMD15572015]. The soil will be packed in a single lift to minimize heterogeneities in the specimen which could arise due to lack of binding between successive lifts. Preparing the specimens in a relatively soft condition also ensures minimal air entrapment. For each mixture, 6 replicate cylinders will be prepared. 
