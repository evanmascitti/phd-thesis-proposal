...
The final sub-section is a brief description of my workflow and data management plan. 
...
Figure __
<!-- \@ref(fig:gantt-chart)  -->
outlines envisaged timelines for the rest of the project. 
...
This section has the most detail of the proposed experiments. 
The development of the method has been thoroughly tested and most of the small pilot experiments are recounted here. 
The level of detail could be viewed as excessive, but I feel this is the appropriate place to document my efforts in validating the method. 
...
2. What measurements best ennumerate infield soil "performance?" 
...
A single critical water content $w_{crit}$ was derived for each soil x effort combination using the Normalized RFI as the surface metric of interest. 
...
This will allow the maximum toughness value to be bracketed by the tests.
...
## Workflow, reproducibility, and data management 

This sub-section briefly outlines the practices I have adopted to ensure my data is stored safely and that the results of my work are correct and reproducible. 
These are rather different than a typical Microsoft Office-based workflow, and they may seem extreme or unnecessary to Office loyalists.
...
To ensure the data is kept in its original state, I never perform any direct operations on the raw files. 
If I discover an error, I correct it with an R script and make a new copy so there is a record of what was done. 
I store my data on a local hard drive which is automatically synced with PSU's subsctiption to Microsoft OneDrive for Business. 
...
This makes sorting dates much easier.
...
I design file names so they convey all relevant information about its contents regardless of the sub-folder in which it resides. 
...
Note that I use leading zeroes before single digits - since we have 12 cylinders, this prevents the computer from sorting the files into something like:
`cyl1`
`cyl10`
`cyl11`
`cyl12`
...
Each is better than nothing, but all have significant flaws. 

I have finally settled on plain text Markdown and R Markdown, for reasons described below in Section \@ref(r-markdown-finished-output). This is a robust system for recording my thoughts and also including images, web links, and photos. 
...
R is a fully-functioning programming language in which relatively complex tasks can be performed. 
...
This lends it a fluency which is less tangible in more general-purpose languages such as C++ or Python.
...
I build all my figures using the **ggplot2** package in R. 
This package is built around the principles outlined in **The Grammar of Graphics** [@Wilkinson2005].
Wilkinson's book describes a theoretical underpinning for building statistical graphics, and **ggplot2** provides an implementation equally suitable for beginners and advanced users. 
...
I somewhat resent this package's documentation and user interface, but I still use it because of its powerful formatting capabilities and because it can output tables to a variety of document formats. 
...
R Markdown is a system linking regular text (i.e. writing) with the source code for an analysis.
This means that there is no need to copy-paste results into a "finished" document. 
This makes the analysis process not only easier, but more likely to be **correct**.

An R Markdown file is a written as a plain text file with no formatting - this is rather different than programs such as Microsoft Word. 
R Markdown instead uses special characters such as `*`, `_` and `#`to provide semantic information about the document's structure and formatting. 
...
Essentially R Markdown translates the formatting instructions you have provided with plain-text markup into other markup languages which require more advanced typesetting instructions (such as \LaTeX, HTML, or xml).
...
A single project might have many finished products which use some (but not all) of the same components.

To address this issue I now use `Make` to manage my workflow.
`Make` is a command-line tool maintained by the GNU project.
There is no graphical user interface - using `Make` requires at least a basic competence with the Unix shell. 
Like R, `Make` is free and open-source. 
...
It then builds a dependency graph and ensures that all target files are kept up to date.
...
If I decide to change the title of one of the figures, or add more data, a traditional Office workflow would require me to build the figure and then copy it into each of the 4 places it is used. 
With `Make`, all I do is make the change and then type `make` at the command line. 
All the relevant documents are updated, but without re-running any of the code which was not affected by the change. 

### Version control

We all fear losing our work in a computer malfunction. 
It can also be useful to store older versions of a project for later reference.
We might employ a manual version control system, such as naming files like 'report-v01.docx`. 
...
A formal version control system such as Git combines these tasks and supplants the need to manually log file versions. 
Most importantly, it automates the bulk of the process - reducing the chance for human error. 

Version control is even more important as the scope of a project increases. 
If we change something in our code, we risk breaking other existing results.
Version control systems allow snapshots of an entire project to be stored and recovered if necessary. 
...
However, now that I am a competent Git user, I could never live without this tool.
...
Currently I am using Git only to collaborate with my "future self." 
...
I was initially skeptical of the investment needed to overhaul my way of working, which was with the standard Office pipeline. 
About a year into my PhD I realized that my existing systems were simply not adequate. 
I began to lose track of experiments and became incensed at the notion of wasting my valuable time on data that might eventually vaporize. 
...
I think they could be tremendously useful to other graduate students. 