...
This paper outlines a new test method to measure the cleat-in/cleat-out effect (Section \@ref(corkboard-effect-section)).
The goal of our test is to identify a single water content at which a given infield soil transitions abruptly from cleat-in cleat-out behavior to a clod-forming failure mode.

The level of detail given in this section could be viewed as excessive, but I feel it is the appropriate place to document my efforts in validating the method. 
It has many steps and each has been validated independently.
Developing a method from scratch is hardly a linear process, but I have done my best to move in a logical order.
In this spirit, I have put many of the detailed calculations to the Appendix.
My intent is to avoid disrupting the flow of the method's description, but to ensure everything is available should the reader wish to verify the details.
...
The next series of sub-sections comprise a detailed description of the method, from creating a soil mixture to a final result. 
...
To make these calculations efficient and reproducible, the above forumlae have been compiled into a set of functions using the R Language for Statistical Computing [@R-core2021].
...
Recall that all of these values for $\rho$ are somewhat higher than those which have been used for other infield soils research (c.f. Figure \@ref(fig:lit-review-density-comparison)).
...
Figure \@ref(fig:proctor-vs-cleat-mark-achieved-density-fig) also clearly shows that the cleat-mark compaction routine packed the soil to a similar but systematically lower dry density than the corresponding Proctor tests.
...
It would appear that the small decrease in dry density should be expected given the specimen size. 
...
1. The "critical" water contents derived from the two methods were very similar (\@ref(tab:derived-water-contents-from-cleat-mark-test-effort-comparison-tab)). Although a statistical comparison was not possible, the differences due to soil type appears to be larger than any difference due to compaction effort. 
...
The prevailing opinion among baseball grounds managers is that the infield should be as compacted as possible. 
...
A 15 cm distance caused excessively rapid drying (initially over 3 % by dry mass per hour). 
Figure \@ref(fig:water-content-change-by-lamp-height)B also shows the rate of water loss was not constant with time. 

At the other extreme, a 45 cm lamp height produced only a small change in water content over 3-4 hr (the typical length of a professional baseball game). 
The 30 cm distance produced the most realistic result. 

Figure \@ref(fig:lamp-height-calibration) shows that the surface temperature observed beneath te 30 cm lamp height compared favorably with that of an actual infield on a sunny August day in Pennsylvania. 
