...
```{r proctor-vs-observed-infield-density, fig.cap='(ref:proctor-vs-observed-infield-density-caption)'}
...
```{r derived-water-contents-from-cleat-mark-test-effort-comparison-tab, fig.cap="The critical volumetric water contents derived from the cleat mark test were nearly identical for the two compaction treatments."}
...
A small intact soil core is removed (depth = 15 mm) using a small soil probe.
...
A 3D scan is then made of the core to allow an accurate computation of its volume.
The volume of the `.ply` object is determined using the `vcgVolume()` function in the **Rvcg** package. 
The volumetric water content of the specimen is determined from its gravimetric water content and dry density:
...
Dirichlet Normal Energy is a relatively complex algorithm often used in anthropology and biology. It is a measure of the total curvature of a surface. Briefly, the vertex normals of each point are averaged ....See @Pampush and @pampush-cited-works for a more complete derivation of DNE.

#### Determination of $w_{crit}$

The goal of the cleat-mark test is to arrive at a single water content where the soil exhibits an abrupt change between brittle and ductile behavior. 
...
Deriving a single point from this curve requires some discussion of the physical meaning behind the data points. 

Moving from wet to dry:

For this discussion NFRI is referenced as the metric of choice, but the general shape of the relationship is the same for other metrics as well. 

When the soil is very wet, the device extends to its maximum strain level. Increasing the water content is not likely to achieve any additional disturbance. 

At lower water contents, disturbance is relatively constant as no soil is removed or fractured from the surface and the new surface area is goverened purely by the shape of the cleats. 

When the soil reaches the crutical transutio it begns to grafment under  the evice's action. 

As the soil continues to dry, the brittleness is maximized and the soil forms chips and clods. These often fragment further and produce additonal surface are . 

When the soil is very dry, the cleats are largely unable to penetrate the soil at the applied load. Little deformation can occur and the NFRI.
