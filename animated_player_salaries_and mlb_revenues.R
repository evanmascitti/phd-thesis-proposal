# These plots are the same ones as the static versions I generated for my thesis
# introduction.....however, these have been altered with gganimate to make them
# dynamic. It is cool to see the time series of revenues and salaries increase!!
# They will be awesome for a presentation!!! 
...
# plot player salaries over time 
...
salaries <- tibble::tibble(
  ...
  tidyr::unnest(value) %>% 
  dplyr::mutate(year = as.double(year)) %>% 
  dplyr::left_join(infl_adj_lookup) %>% 
  dplyr::mutate(infl_adj_salary = salary*(index_cpi/cpi)) %>% 
...
salary_distribution_animated_plot <- salaries  %>%
  ggplot2::ggplot(ggplot2::aes(x = infl_adj_salary))+
  ...
  ggplot2::scale_x_continuous(name = expression(Annual~player~salary~(log[10]~scale)),
                              trans= 'log10',
                              limits= c(5*10^4, 5*10^7),
                              breaks = 5*10^c(5:7),
                              labels = scales::label_dollar())+
  ggplot2::labs(title = '{closest_state} MLB player salaries',
...
gganimate::animate(mlb_revenues_plot, fps = 10)
animate(salary_distribution_animated_plot, fps = 10)
